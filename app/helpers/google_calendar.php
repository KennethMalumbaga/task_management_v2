...
if (!function_exists('google_calendar_create_meeting_event')) {
    function google_calendar_create_meeting_event($accessToken, array $payload)
    {
        ...
        if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $meetingDate)) {
            return ['ok' => false, 'event' => null, 'error' => 'Meeting date is invalid.'];
        }
        if (!preg_match('/^\d{2}:\d{2}$/', $startTime) || !preg_match('/^\d{2}:\d{2}$/', $endTime)) {
            return ['ok' => false, 'event' => null, 'error' => 'Meeting start and end time are required.'];
        }
        ...
        try {
            ...
            return ['ok' => false, 'event' => null, 'error' => 'Meeting date or time could not be parsed.'];
        }

        if ($endAt <= $startAt) {
            return ['ok' => false, 'event' => null, 'error' => 'Meeting end time must be after the start time.'];
        }

        $sourceUrl = APP_URL . '/calendar.php?date=' . rawurlencode($meetingDate) . '&month=' . rawurlencode($startAt->format('m')) . '&year=' . rawurlencode($startAt->format('Y'));
        ...
        $body = [
            ...
            'start' => [
                'dateTime' => $startAt->format(DATE_RFC3339),
                'timeZone' => $timezone,
            ],
            'end' => [
                'dateTime' => $endAt->format(DATE_RFC3339),
                'timeZone' => $timezone,
            ],
            'conferenceData' => [
                'createRequest' => [
