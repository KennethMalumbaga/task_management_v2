...
function get_top_rated_users($pdo, $limit = 5)
{
    ...
    $fetchByUserId = function ($sql, $params, $keyColumn) use ($pdo) {
        $stmt = $pdo->prepare($sql);
        $stmt->execute($params);
        $map = [];
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
            $userId = (int)($row[$keyColumn] ?? 0);
            if ($userId > 0) {
                $map[$userId] = $row;
            }
        }
    ...
    $sqlTaskRatings = "SELECT ta.user_id, COUNT(*) AS count, AVG(t.rating) AS avg
                       FROM task_assignees ta
                       JOIN tasks t ON t.id = ta.task_id
                       WHERE t.status = 'completed' AND t.rating > 0";
    $paramsTaskRatings = [];
    $taskScope = tenant_get_scope($pdo, 'tasks', 't');
    $sqlTaskRatings .= $taskScope['sql'];
    $paramsTaskRatings = array_merge($paramsTaskRatings, $taskScope['params']);
    $assigneeScope = tenant_get_scope($pdo, 'task_assignees', 'ta');
    $sqlTaskRatings .= $assigneeScope['sql'];
    $paramsTaskRatings = array_merge($paramsTaskRatings, $assigneeScope['params']);
    $sqlTaskRatings .= " GROUP BY ta.user_id";
    $taskRatingMap = $fetchByUserId($sqlTaskRatings, $paramsTaskRatings, 'user_id');

    $sqlLeaderTasks = "SELECT user_id, COUNT(*) AS count
                       FROM task_assignees
                       WHERE role = 'leader'";
    [$sqlLeaderTasks, $paramsLeaderTasks] = user_model_append_scope($pdo, $sqlLeaderTasks, [], 'task_assignees');
    $sqlLeaderTasks .= " GROUP BY user_id";
    $leaderTaskMap = $fetchByUserId($sqlLeaderTasks, $paramsLeaderTasks, 'user_id');

    $leaderAdminMap = [];
    if (tenant_column_exists($pdo, 'task_assignees', 'performance_rating')) {
        $sqlLeaderAdmin = "SELECT user_id, COUNT(*) AS count, AVG(performance_rating) AS avg
                           FROM task_assignees
                           WHERE role = 'leader'
                             AND performance_rating IS NOT NULL
                             AND performance_rating > 0";
        [$sqlLeaderAdmin, $paramsLeaderAdmin] = user_model_append_scope($pdo, $sqlLeaderAdmin, [], 'task_assignees');
        $sqlLeaderAdmin .= " GROUP BY user_id";
        $leaderAdminMap = $fetchByUserId($sqlLeaderAdmin, $paramsLeaderAdmin, 'user_id');
    }

    $leaderPeerMap = [];
    if (tenant_table_exists($pdo, 'leader_feedback')) {
        $sqlLeaderPeer = "SELECT leader_id, COUNT(*) AS count, AVG(rating) AS avg
                          FROM leader_feedback
                          WHERE rating IS NOT NULL";
        [$sqlLeaderPeer, $paramsLeaderPeer] = user_model_append_scope($pdo, $sqlLeaderPeer, [], 'leader_feedback');
        $sqlLeaderPeer .= " GROUP BY leader_id";
        $leaderPeerMap = $fetchByUserId($sqlLeaderPeer, $paramsLeaderPeer, 'leader_id');
    }

    $memberSubtaskMap = [];
    if (tenant_table_exists($pdo, 'subtasks') && tenant_column_exists($pdo, 'subtasks', 'score')) {
        $sqlMemberSubtasks = "SELECT member_id, COUNT(*) AS count, AVG(score) AS avg
                              FROM subtasks
                              WHERE score IS NOT NULL";
        [$sqlMemberSubtasks, $paramsMemberSubtasks] = user_model_append_scope($pdo, $sqlMemberSubtasks, [], 'subtasks');
        $sqlMemberSubtasks .= " GROUP BY member_id";
        $memberSubtaskMap = $fetchByUserId($sqlMemberSubtasks, $paramsMemberSubtasks, 'member_id');
    }

    $blendLeaderScores = function ($adminAvg, $peerAvg) {
        $hasAdmin = $adminAvg !== null;
        $hasPeer = $peerAvg !== null;
        if ($hasAdmin && $hasPeer) {
            return ((float)$adminAvg + (float)$peerAvg) / 2;
        }
        if ($hasAdmin) {
            return (float)$adminAvg;
        }
        if ($hasPeer) {
            return (float)$peerAvg;
        }
        return null;
    };
    ...
    foreach ($users as $u) {
        $userId = (int)($u['id'] ?? 0);
        $taskStats = $taskRatingMap[$userId] ?? [];
        $rated_task_count = (int)($taskStats['count'] ?? 0);
        $avg_task_rating = $rated_task_count > 0 && isset($taskStats['avg']) ? (float)$taskStats['avg'] : null;

        $leaderTaskCount = (int)($leaderTaskMap[$userId]['count'] ?? 0);
        if ($leaderTaskCount > 0) {
            $adminCount = (int)($leaderAdminMap[$userId]['count'] ?? 0);
            $peerCount = (int)($leaderPeerMap[$userId]['count'] ?? 0);
            $adminAvg = $adminCount > 0 && isset($leaderAdminMap[$userId]['avg']) ? (float)$leaderAdminMap[$userId]['avg'] : null;
            $peerAvg = $peerCount > 0 && isset($leaderPeerMap[$userId]['avg']) ? (float)$leaderPeerMap[$userId]['avg'] : null;
            $collab_score_count = $adminCount + $peerCount;
            $avg_collab_rating = $collab_score_count > 0 ? $blendLeaderScores($adminAvg, $peerAvg) : null;
        } else {
            $memberStats = $memberSubtaskMap[$userId] ?? [];
            $collab_score_count = (int)($memberStats['count'] ?? 0);
            $avg_collab_rating = $collab_score_count > 0 && isset($memberStats['avg']) ? (float)$memberStats['avg'] : null;
        }
