...
function count_all_group_unread($pdo, $user_id)
{
    ...
    $readParams = [$user_id];
    $readSql = "SELECT group_id, user_id, MAX(last_message_id) AS last_message_id
                FROM group_message_reads
                WHERE user_id = ?";
    ...
    $readSql .= " GROUP BY group_id, user_id";

    $params = $readParams;

    $sql = "SELECT COUNT(gmsg.id)
            FROM `groups` g
            JOIN group_members gm ON gm.group_id = g.id
            JOIN group_messages gmsg ON gmsg.group_id = g.id
            LEFT JOIN ({$readSql}) gmr ON gmr.group_id = g.id AND gmr.user_id = gm.user_id
            WHERE gm.user_id = ?
              AND gmsg.id > COALESCE(gmr.last_message_id, 0)";
    $params[] = $user_id;
    ...
    $sql .= $scope['sql'];
    $params = array_merge($params, $scope['params']);
    $scope = tenant_get_scope($pdo, 'group_messages', 'gmsg');
    $sql .= $scope['sql'];
    $params = array_merge($params, $scope['params']);
