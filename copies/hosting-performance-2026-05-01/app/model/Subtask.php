...
function get_task_started_subtask_map($pdo, array $task_ids)
{
    subtask_ensure_schema($pdo);
    ...
    $startedStatuses = ['submitted', 'completed', 'in_progress', 'revise', 'revision_needed', 'rejected'];
    $taskPlaceholders = implode(',', array_fill(0, count($task_ids), '?'));
    $statusPlaceholders = implode(',', array_fill(0, count($startedStatuses), '?'));
    $params = array_merge($task_ids, $startedStatuses);

    $sql = "SELECT s.task_id, COUNT(*) AS started_count
            FROM subtasks s
            WHERE s.task_id IN ({$taskPlaceholders})
              AND LOWER(TRIM(COALESCE(s.status, 'pending'))) IN ({$statusPlaceholders})";
    [$sql, $params] = subtask_append_scope($pdo, $sql, $params, 'subtasks', 's');
    $sql .= " GROUP BY s.task_id";
    ...
    foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
        ...
        if ($taskId > 0) {
            $map[$taskId] = ((int)($row['started_count'] ?? 0)) > 0;
        }
    }
